 ...
 */
public class Main {
    private static final String counterFileName = "counter.dat";
    ...
    private static void updateExecutionCounter() {
        int counter = 0;
        Path counterPath = Path.of(counterFileName);

        // Read existing counter if file exists
        if (Files.exists(counterPath)) {
            try (var in = new DataInputStream(Files.newInputStream(counterPath))) {
                counter = in.readInt();
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        try (var out = new DataOutputStream(Files.newOutputStream(counterPath))) {
            out.writeInt(counter);
        } catch (IOException e) {
            e.printStackTrace();
        }

        System.out.println("Program has been executed " + counter + " times.");
    }
    ...
    public static void main(String[] args) {
        updateExecutionCounter(); // <--- ADD THIS LINE
